...
class Qimen:
    ...
    def _qin_gong(self, option):
        """时家：找到禽/芮所在宫位（用九星盘）"""
        # 你的 pan_star 返回 (宫->星 的 dict, 星->宫 的 dict)
        star_pan = config.pan_star(self.year, self.month, self.day, self.hour, self.minute, option)[0]
        for gong, star in star_pan.items():
            if star in ("禽", "芮"):
                return gong
        return None

    def _sky_for_reading(self, option):
        """
        时家：生成【解读用天盘】：
        - 原天盘去掉“中宫”
        - 将“中宫天干”作为“寄干”附加到【禽宫】的列表中
        - 结构：{宫: {'天干': 原天盘干, '寄干': [可能为空或有一个]}}
        """
        sky = self.pan_sky(option)
        if isinstance(sky, tuple):
            sky = sky[0]  # 有些分支你返回了 (dict, dict)，这里统一成 dict

        sky = dict(sky)
        center_gan = sky.get("中")
        qin_gong = self._qin_gong(option)

        result = {}
        for gong, gan in sky.items():
            if gong == "中":
                continue
            result[gong] = {"天干": gan, "寄干": []}

        if center_gan and qin_gong and qin_gong in result:
            result[qin_gong]["寄干"].append(center_gan)

        return result

    def _qin_gong_minute(self, option):
        """刻家：找到禽/芮所在宫位"""
        star_pan = config.pan_star_minute(self.year, self.month, self.day, self.hour, self.minute, option)[0]
        for gong, star in star_pan.items():
            if star in ("禽", "芮"):
                return gong
        return None

    def _sky_for_reading_minute(self, option):
        """刻家：生成【解读用天盘】（把中宫寄到禽宫）"""
        sky = config.pan_sky_minute(self.year, self.month, self.day, self.hour, self.minute)
        sky = dict(sky)
        center_gan = sky.get("中")
        qin_gong = self._qin_gong_minute(option)
    ...
    def pan(self, option):#1拆補 #2置閏
        ...
        star_pan = config.pan_star(self.year,
                                          self.month,
                                          self.day,
                                          self.hour,
                                          self.minute,
        ...
        sky_raw = self.pan_sky(option)  # 原天盘（保留）
        sky_reading = self._sky_for_reading(option)  # 解读用
        qin_g = self._qin_gong(option)
        ...
        return {
            ...
            "天盤_解讀用": self._sky_for_reading(option),
            "寄宮": {
                "禽宮": self._qin_gong(option),
                "中宮寄干": self._sky_for_reading(option).get(self._qin_gong(option), {}).get("寄干", [])
            },
    ...
    def pan_minute(self, option):
        ...
        zfzs = config.zhifu_n_zhishi_ke(self.year,
                                        ...
                                        self.hour,
                                        self.minute)
        star_pan = config.pan_star_minute(self.year,
                                                 self.month,
                                                 self.day,
                                                 self.hour,
                                                 self.minute,
        ...
        return {
            ...
            "地盤": config.pan_earth_minute(self.year,
                                          ...
                                          self.hour,
                                          self.minute),
            "天盤_解讀用": self._sky_for_reading_minute(option),
            "寄宮": {
                "禽宮": self._qin_gong_minute(option),
                "中宮寄干": self._sky_for_reading_minute(option).get(self._qin_gong_minute(option), {}).get("寄干", [])
            },                    
            "門": door,
            "星": star_pan,
            "神": god,
    ...
    def jixing(self, option=1):
        ...
        result = {
            gan:(True,gong)
            for gong, gan in sky_pan.items()
            if gan in jixing_rules and gong == jixing_rules[gan]
        }
        return result if result else '无天盘干击刑'

    def menpo(self, option=1):
        """
        门迫判断
        """
        door_wuxing = {'休': '水', '生': '土', '伤': '木', '杜': '木', 
                       '景': '火', '死': '土', '惊': '金', '驚': '金', '开': '金', '開': '金',
                       '傷':'木'
        }
        gong_wuxing = {
            '坎': '水', '艮': '土', '震': '木', '巽': '木',
        ...
        door_pan = config.pan_door(self.year, self.month, self.day, self.hour, self.minute, option)
        result = {}
        for gong, door in door_pan.items():
            if door == '中':
                continue
            men_wx = door_wuxing[door]
            gong_wx = gong_wuxing[gong]
            is_menpo = ke_relation.get(men_wx) == gong_wx
            if is_menpo:
                result[gong] = (door, f"{men_wx}克{gong_wx}")
        return result

    def fuyin(self, option=1):
        """
        伏吟判断
        """
        star_pan = config.pan_star(self.year, self.month, self.day, self.hour, self.minute, option)[0]
        door_pan = config.pan_door(self.year, self.month, self.day, self.hour, self.minute, option)
        star_fuyin = any(star == '心' and gong == '乾' for gong, star in star_pan.items())
        door_fuyin = any(door == '開' and gong == '乾' for gong, door in door_pan.items())
        result = {}
        if star_fuyin:
            result['星伏吟'] = True
        if door_fuyin:
            result['门伏吟'] = True
        if star_fuyin and door_fuyin:
            result['星门俱伏'] = True
        return result if result else '无'

    def fanyin(self, option=1):
        """
        反吟判断
        """
        star_pan = config.pan_star(self.year, self.month, self.day, self.hour, self.minute, option)[0]
        door_pan = config.pan_door(self.year, self.month, self.day, self.hour, self.minute, option)
        star_fanyin = any(star == '心' and gong == '巽' for gong, star in star_pan.items())
        door_fanyin = any(door == '開' and gong == '巽' for gong, door in door_pan.items())
        result = {}
        if star_fanyin:
            result['星反吟'] = True
        if door_fanyin:
            result['门反吟'] = True
        if star_fanyin and door_fanyin:
            result['星门俱反'] = True
        return result if result else '无'

    def tiangan_rumu(self, option=1):
        """
        天干入墓判断
        """
        rumu_rules = {
        ...
        result = {
            gan: (True, gong)
            for gong, gan in sky_pan.items()
            if gan in rumu_rules and gong == rumu_rules[gan]
        }
        return result if result else '无天盘干入墓'
